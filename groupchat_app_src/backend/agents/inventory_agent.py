...
class InventoryAgent(BaseAgent):
    ...
    async def can_handle(self, user_message: str, context: Dict[str, Any]) -> tuple[bool, float]:
        ...
        high_confidence_patterns = [
            r"(check|what's|show|get|tell).*(stock|inventory|supplies)",
            r"(low on|short on|need|out of|running low)",
            r"how many .* (do we have|available|in stock)",
            r"(order|purchase|buy|get more)",
    ...
    async def execute(self, user_message: str, context: Dict[str, Any], session: AsyncSession) -> Dict[str, Any]:
        ...
        # Check for low stock / restocking queries first
        if re.search(r"(low on|short on|need|out of|running low|restock|restocking|need to (order|reorder)|what.*need)", message_lower):
            return await self._handle_low_stock_alert(user_message, session)
        
        elif re.search(r"(check|what's|show|get|tell).*(stock|inventory)", message_lower):
        ...
        elif re.search(r"(add|new|register) .* (item|material|supply)", message_lower):
            return await self._handle_add_item(user_message, session)
        
        else:
            # General inventory query
            return await self._handle_general_query(user_message, session)
    ...
    async def _handle_add_item(self, message: str, session: AsyncSession) -> Dict[str, Any]:
        ...
        return {
            "success": False,
            "message": "To add items, please use the format: 'Add [item name], quantity: [number], category: [category]'",
            "data": None,
    ...
    def _extract_item_name(self, message: str) -> Optional[str]:
        ...
        if re.search(r"(what|which).*(need|low|restock)", message_lower) and not re.search(r"\b(pencil|marker|beaker|arduino|kit|box|pack)\b", message_lower):
            return None
        
        # Common patterns for specific items
        patterns = [
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                stop_words = {"the", "a", "an", "some", "more", "any", "all", "everything", "complete", "entire", "full", "me", "restocking", "restock", "need", "items", "things", "stuff"}
